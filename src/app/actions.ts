...
export async function processCnpjs(cnpjs: string[], referenceMonth: string) {
  try {
    const response = await fetch("http://localhost:4031/process-cnpjs", {
      method: "POST",
      headers: {
...
export async function getTaskStatus(taskId: string) {
  try {
    const response = await fetch(`http://localhost:4031/task/${taskId}`, {
      credentials: "include"
    });
...
export async function downloadDas(taskId: string) {
  try {
    const response = await fetch(`http://localhost:4031/download/${taskId}`, {
      credentials: "include"
    });
